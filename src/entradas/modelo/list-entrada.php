<?php
    ...
    if($conexao){
        ...
        if(!empty($requestData['draw']) || isset($requestData['draw'])){
            ...
            $sql = "SELECT id_entrada, responsavel, DATE_FORMAT (data_entrada, '%d/%m/%Y') as data_entrada, quantidade, custo, pacotes FROM entradas WHERE 1=1";
            ...
            if(!empty($filtro)){
                $sql .= " AND (id_entrada LIKE '$filtro%' ";
                $sql .= " OR nome_respo LIKE '$filtro%') "; 
            }
